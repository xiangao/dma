...
Both `crumble` and `dma` target the same mediation estimands. The practical
difference is in the nuisance fitting:

- **crumble** uses `mlr3superlearner`, which ensembles many classical ML
  learners (GLM, random forest, MARS, etc.). This gives more modeling choices,
  but also more packages and tuning choices.
- **dma** uses `engression`, a single neural network that learns the
  full conditional distribution. This keeps the dependency structure narrower,
  but requires torch.
