dma_data <- S7::new_class("dma_data",
	properties = list(
		data = S7::new_property(S7::class_data.frame),
		vars = S7::new_property(S7::new_class("dma_vars")),
		...
		ref_levels = S7::new_property(S7::class_list, default = list())
	),
	constructor = function(data, vars, weights, d0, d1) {
		# Capture factor levels from full dataset for consistent OHE across folds
		ref_levels <- list()
		for (v in names(data)) {
			if (is.factor(data[[v]]) || is.character(data[[v]])) {
				ref_levels[[v]] <- if (is.factor(data[[v]])) levels(data[[v]]) else sort(unique(data[[v]]))
			}
		}
...
S7::method(training, dma_data) <- function(x, fold_obj, fold) {
	list(
		...
		weights = x@weights[fold_obj[[fold]]$training_set]
	)
}

S7::method(validation, dma_data) <- function(x, fold_obj, fold) {
	list(
		...
		weights = x@weights[fold_obj[[fold]]$validation_set]
	)
}
