test_that("one_hot_encode produces consistent columns with ref_levels", {
  ...
  ohe1 <- dmaR:::one_hot_encode(df1, c("x", "y"), ref_levels = ref_levels)
  ohe2 <- dmaR:::one_hot_encode(df2, c("x", "y"), ref_levels = ref_levels)
...
test_that("one_hot_encode works without ref_levels", {
  ...
  ohe <- dmaR:::one_hot_encode(df, c("x", "y"))
...
test_that("prepare_engression_x drops constant columns for training", {
  ...
  result <- dmaR:::prepare_engression_x(df, c("x", "y", "z"))
...
test_that("prepare_engression_x aligns to ref_cols for prediction", {
  ...
  ref <- c("x", "y", "z")
  result <- dmaR:::prepare_engression_x(df, c("x", "y"), ref_cols = ref)
...
test_that("is_binary correctly identifies binary vectors", {
  expect_true(dmaR:::is_binary(c(0, 1, 0, 1)))
  expect_true(dmaR:::is_binary(c(0, 0, 0)))
  expect_false(dmaR:::is_binary(c(0, 1, 2)))
  expect_false(dmaR:::is_binary(c(0.5, 1)))
})

test_that("normalize produces mean-1 weights", {
  x <- c(2, 4, 6)
  result <- dmaR:::normalize(x)
...
test_that("shift_data modifies treatment column", {
  ...
  result <- dmaR:::shift_data(df, "sports", NA_character_, d0)
