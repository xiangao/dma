...
test_that("tidy returns a data.frame with correct columns", {
  ...
  result <- dmaR::dma(
    ...
    control = dmaR::dma_control(crossfit_folds = 1L, num_epochs = 1L, riesz_epochs = 1L)
  )

  td <- dmaR:::tidy.dma_result(result)
...
test_that("one-hot encoding is consistent across folds", {
  ...
  ohe1 <- dmaR:::one_hot_encode(
    cd@data[1:2, ], "A", ref_levels = cd@ref_levels
  )
  ohe2 <- dmaR:::one_hot_encode(
